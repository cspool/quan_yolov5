...
from models.quan_util import quan_Conv2d_BNFold
...
def run(
    ...
    if not training:
        ...
        for m in model.modules():
            if isinstance(m, quan_Conv2d_BNFold):
                m.layer_num_print = 0
    ...
    # 2024-09-19 add flops params
    if not training:
        from models.quan_util import flops, params, layer_info
    layer_num = 1
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if batch_i == 0 and not training:
            for m in model.modules():
                if isinstance(m, quan_Conv2d_BNFold):
                    m.layer_num_print = layer_num
                    layer_num += 1
